...
const questionsTree = [{
        ...
        question: [{
            ...
            {
                qustion: 'Любишь многозадачность, а на твоём компьютере одновременно могут быть запущены несколько программ?',
		...
		question: [
			{
                ...
                answers: ["Да", "Нет"],
                cons_answers: ["При чём тут IT? Расскажу чуть позже&nbsp;😉", "Хорошо, значит, этот вариант предлагать не&nbsp;буду."]
            },
			{
				...
				cons_answers: ['Так и запишем&nbsp;🖌', 'Чем лучше ты знаешь себя, тем проще сделать выбор&nbsp;😎 И это касается не только IT']
			},
            {
...
$(document).ready(function() {
    ...
    $(document).on('click', '.chat-answer__inner', function() {
        ...
        if (ansIndex == 1) {
            if(categoryNow === (questionsTree.length - 1)){
                lastCat = true;
            } else goToNextCategory = true;
        } else {
            goToNextCategory = false;
            if (!$('.chat-answers').hasClass('chat-answers__single')) categories[categoryNow].result++;
            // categories[categoryNow].result === questionsTree[categoryNow].question.length && console.log((categories[categoryNow]).category_name);
        }
...
function showNextQuestion() {
    ...
    if (questionNow >= categoryObj.question.length || goToNextCategory ) {
        if (categories[categoryNow].result === questionsTree[categoryNow].question.length || lastCat) {

            let nothingChoose = false;
            if(lastCat){
                nothingChoose = true;
                categoryNow = categories.length - 1;
            }
            let resultHtml = `<div class="chat-message__bubble"><div class="chat-message__creative"><div class="chat-message__image"><img src="${(categories[categoryNow]).image}" alt="image"/></div><div class="chat-message__title"> ${(categories[categoryNow]).title} </div></div><div class="chat-message__text"><p> ${(categories[categoryNow]).text} </p>`;

            if (!nothingChoose) {
                allDone = true;
                resultHtml += `<p><span class="blue">#ITGIRLSCHOOL</span> — онлайн-школа программирования для девушек. Помогаем обрести интересную востребованную профессию и обеспечить благополучное будущее.</p></div><div class="chat-message__button"><a href="${(categories[categoryNow]).link}" class="button">Подробнее</a></div>`;
            } else {
                resultHtml += `</div><div class="chat-message__button"><a href="
                ${(categories[categoryNow]).link}" class="button">Попробовать ещё раз</a></div>`;
            }
            resultHtml += '</div>';
